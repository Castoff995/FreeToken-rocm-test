...
__always_inline __device__ auto load_nc(const uint1* __restrict__ src) -> uint1 {
    ...
    // patched: PTX ld.global.nc not available on AMD; plain volatile load
    const uint1 *s1 = src;
    return *s1;
#else
    uint32_t tmp;
...
#if defined(__HIP_PLATFORM_AMD__)
    *reinterpret_cast<uint1 *>(dst) = value;  // patched: st.global.wt fallback
#else
...
#if defined(__HIP_PLATFORM_AMD__)
    *reinterpret_cast<uint2 *>(dst) = value;
#else
...
#if defined(__HIP_PLATFORM_AMD__)
    *reinterpret_cast<uint4 *>(dst) = value;
#else
    uint32_t tmp0 = value.x;
    uint32_t tmp1 = value.y;
